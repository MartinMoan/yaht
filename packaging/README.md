...
- `dist/H5Viewer/` -- a folder ("onedir") containing `H5Viewer.exe`
  (Windows) / `H5Viewer` (Linux) plus everything it needs alongside it in
...
- `dist/H5Viewer-standalone.exe` (Windows) / `dist/H5Viewer-standalone`
...
- A custom app icon -- there's no `.ico`/`.png` app icon in this repo yet
  ...
  (Windows) to the `EXE(...)` call in `h5tools.spec` once one exists.
